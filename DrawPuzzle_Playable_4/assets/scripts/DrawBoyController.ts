...
export default class DrawBoyController extends cc.Component {
    ...
    onTouchEnd(event) {
        ...
        // var dis = this.boyDrawCollider.position.x - this.defaultPos.x;

        console.log("waypoints: " + this.wayPoints.length);

        if (this.boyDrawPoint >= 4 && this.borderIsTouched == false && this.drawOut == false) {
        ...
        else if (this.boyDrawPoint < 4 || this.borderIsTouched == true || this.drawOut == true) {
            this.graphics.clear();
    ...
    update(dt: number) {
        if (this.isMoving == true && this.manager.goalPoint >= 2) {
            this.MoveChar();
        }
    }
}
